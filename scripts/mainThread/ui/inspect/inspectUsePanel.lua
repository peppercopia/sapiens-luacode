...
local function updateConfirmButton()
    if currentConstructableTypeIndex then
        ...
        if mainViewHasFocus then
            uiStandardButton:setTextWithShortcut(confirmButton, classificationType.actionName, "game", "confirm", eventManager.controllerSetIndexMenu, "menuSelect")
        else
            uiStandardButton:setTextWithShortcut(confirmButton, classificationType.actionName, "game", "confirm", eventManager.controllerSetIndexMenu, "menuCancel")
        end
    end
end
...
local function selectButton(buttonInfo)
    if buttonInfo.gridButtonData then
        ...
        confirmFunction = function()
            if constructableType.classification == constructable.classifications.build.index or
            constructableType.classification == constructable.classifications.path.index or
            constructableType.classification == constructable.classifications.plant.index or
            constructableType.classification == constructable.classifications.place.index then
                buildModeInteractUI:show(currentConstructableTypeIndex, false, false)
            elseif constructableType.classification == constructable.classifications.craft.index then
                local planInfo = {
                    ...
                    craftResourcePlanObjectID = currentObjectInfo.uniqueID,
...
function inspectUsePanel:load(inspectUI_, inspectObjectUI, world_, parentContainerView)
...
local currentObjectTypeIndex = nil
...
local function reconstructGridView()
    ...
    if gameObject.types[currentObjectTypeIndex].isStorageArea then
        local addedConstructableTypes = {}
        constructables = {}
        local inventory = currentObjectInfo.sharedState.inventory
        if inventory and inventory.countsByObjectType then
            for objectTypeIndex, count in pairs(inventory.countsByObjectType) do
                if count > 0 then
                    local thisObjectConstructables = constructable.constructablesByResourceObjectTypeIndexes[objectTypeIndex]
                    if thisObjectConstructables then
                        for j,constructableTypeIndex in ipairs(thisObjectConstructables) do
                            if not addedConstructableTypes[constructableTypeIndex] then
                                table.insert(constructables, constructableTypeIndex)
                                addedConstructableTypes[constructableTypeIndex] = true
                            end
                        end
                    end
                end
...
function inspectUsePanel:show(object)
    currentObjectInfo = object
    local objectTypeIndex = object.objectTypeIndex
    if currentObjectTypeIndex ~= objectTypeIndex or gameObject.types[objectTypeIndex].isStorageArea then
        --hasMadeDefaultSelection = false
        currentObjectTypeIndex = objectTypeIndex
        reconstructGridView()
    end
    ...
    inspectUI:setModalPanelTitleAndObject(gameObject:getDisplayName(object), object)
end

function inspectUsePanel:popUI()
    if requiredResourcesScrollViewHasFocus then
