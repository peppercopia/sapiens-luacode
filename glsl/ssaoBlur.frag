...
//#define SSAO_STRENGTH_POWER 1.45
//#define SSAO_STRENGTH_POWER 1.35
#define SSAO_STRENGTH_POWER 1.35
...
float getVertical(vec2 inPos)
{
    float result = texture(inputMap, inPos).r * weight[0];
    vec2 texelSize = 0.7 / ubo.screenSize.xy;

    for (int i=1; i<3; i++) {
        vec2 offsetValue = vec2(0.0, offset[i] * texelSize.y);
        result += texture(inputMap, (inPos + offsetValue)).r * weight[i];
        result += texture(inputMap, (inPos - offsetValue)).r * weight[i];
    }
...
void main() {
    ...
    vec2 texelSize = 0.7 / ubo.screenSize.xy;
    float result = getVertical(outPos) * weight[0];
    ...
    outColor = vec4(vec3(pow(result, SSAO_STRENGTH_POWER)), 1.0);
    //outColor = vec4(vec3(result), 1.0);
}
