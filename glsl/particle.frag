...
layout(location = 2) in vec2 lifeColorOffset;
layout(location = 3) in vec3 outWorldPos;
layout(location = 4) in vec3 outSunPos;
layout(location = 5) in vec4 outLightAddition_cloudCover;
layout(location = 6) in vec4 outShadowCoords[4];
layout(location = 10) in vec4 outRainDepthCoords;

layout(location = 0) out vec4 data;

void main()
{
    vec4 tex = texture(texMap, outTexCoord * vec2(0.125, -0.109375) + texOffset);
    vec4 texLife = texture(texMap, lifeColorOffset);
    vec4 combined = tex * texLife;
    combined.rgb = combined.rgb * combined.rgb * 0.3;
    ...
    data.a = combined.a;
    data.rgb = hdrFinal(exposureSampler, max(diffuseLit, vec3(0.0,0.0,0.0)));
