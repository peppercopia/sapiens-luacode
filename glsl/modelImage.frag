...
layout(binding = 4) uniform sampler2D texImage;
layout(binding = 5) uniform sampler2D normalImage;
...
void main(void)
{
    vec3 normalTex = normalize((texture(normalImage, outTex).rgb - vec3(0.5,0.5,0.5)) * vec3(1.0,1.0,0.5));

    vec3 normalToUse = (outNormalMatrix * vec4(normalTex, 1.0)).xyz;

    vec4 tex = texture(texImage, outTex);
  ...
  //  vec3 litColor = diffuseLit + (sampledColor) * reflectance;
    ...
    data = result * tex * vec4(1,1,1,outColor.a);
