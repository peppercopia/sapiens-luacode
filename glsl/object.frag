...
void main(void)
{
    ...
    if(viewDepth > 0.0)
    {
        float viewAngle = dot(outView / outViewLength, normalize(outWorldPos.xyz));
        //viewDepth = viewDepth / max(viewAngle, 0.001);
        viewDepth = viewDepth / max(0.8 + viewAngle * 0.2, 0.001);

        worldPosToUse = outWorldPos.xyz + viewDepth * (outView / outViewLength * 8.388608);
        worldViewVecToUse = (outWorldPos.xyz - outWorldCamPos);
        
        if(viewDepth > 0.0)
        {
            //viewDepth = pow(viewDepth * 6.0, 0.6);
   ...
   // vec3 normalToUse = normalize(outNormal);
    ...
    vec3 outBinormal = normalize(cross(normalize(outTangent), outNorm));
    vec3 finalTangent = normalize(cross(outNorm, outBinormal));*/
   ...
   // diffuseLit += colorToUse * getSpotlight(V, outView, normalToUse, ubo.camDirection.xyz);
    ...
    if(ubo.outputType_SSAOEnabled.x != 2)
    {
        //vec3 waterDiffuseLit = vec3(0.0, 0.04, 0.06) * (sunColor * clamp(dot( normalizedWorldPosition, ubo.sunPos.xyz ), 0.0, 1.0) + diffuseColor);
    ...
    //data.rgb = outPos * vec3(0.1) + vec3(0.5);

    //data.rgb = vec3(0.5 + outWorldPos.w);

    //data.rgb = normalToUse * vec3(0.5) + vec3(0.5);
