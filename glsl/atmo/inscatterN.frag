...
layout(binding = 4) uniform sampler2D deltaJSampler[32];
...
{
    float texZ = fract(clamp(texCoord.z, 0.0, 1.0)) * 32.0;
    int base = int(texZ);
...
void main() {
    ...
    data.rgb = inscatter(inscatterUBO.rAndFirst.x, mu, muS, nu);
    data.a = 1.0;
}
