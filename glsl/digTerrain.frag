...
void main()
{
    ...
    vec4 gradientColor = mix(vec4(0.0,0.0,0.0,0.1), vec4(1.0,1.0,1.0,0.1), smoothstep(0.323,0.343, fraction) * (1.0 - smoothstep(0.657, 0.677, fraction)));

    data = mix(gradientColor, vec4(1.0), line);
    data = mix(data,  vec4(0.0,0.0,1.0, 1.0), inside * 0.2) * max(inside,  (1.0 - outHeightFlatAnimation.y));
}
