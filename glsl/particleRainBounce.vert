...
layout(binding = 15) uniform sampler2D rainDepthTextureStandard;
...
layout(location = 10) out float outAlpha;
...
void main()
{
    //vec4 groundPosTexLookup = ubo.rainMatrix * vec4(position, 1.0);
    vec4 rainDepthCoords = ubo.rainMatrix * vec4(position, 1.0);

    vec4 rainDepthValue = texture(rainDepthTextureStandard, rainDepthCoords.xy);

    vec4 foo = vec4(rainDepthCoords.x, rainDepthCoords.y, rainDepthValue.x, 1.0);

    
    vec3 groundPos = vec3(inverse(ubo.rainMatrix) * foo);

    vec3 up = normalize(ubo.origin.xyz);
    vec3 back = normalize(groundPos.xyz - (camera.camOffsetPos.xyz + ubo.camPosLocal.xyz));
    vec3 right = normalize(cross(up, back));
    up = normalize(cross(right, -back));

    float size = particleSize * typeAndLifeLeftAndRandomValueAndScale.w * (1.0 + (1.0 - typeAndLifeLeftAndRandomValueAndScale.y) * 8.0);
    //float yOffsetFraction = 
    float yPosOffset = 0.0005 * (1.0 + typeAndLifeLeftAndRandomValueAndScale.y * 0.5);
    vec2 vertPos = vec2(-0.5 + texCoord.x, 0.0 + texCoord.y);
    vec3 offsetPos = groundPos.xyz + right * vertPos.x * size + up * vertPos.y * size + up * yPosOffset;
    ...
    texOffset = vec2(posOffsetFract,1.0 - yOffset - 0.125);
    ...
    outAlpha = typeAndLifeLeftAndRandomValueAndScale.y;
}
